...
Individual run_genetic_algorithm(const IRP& irp, const GA_Params& ga_params, const ACO_Params& aco_params, bool verbose) {
    ...
    // Contadores de estagnação foram removidos
    ...
    for (int gen = 0; gen < ga_params.nGen; ++gen) {
        ...
        // --- NOVA ESTRATÉGIA DE SELEÇÃO 10% / 70% / 20% ---

        // 1. (10%) Elitismo
        // Ordena a população atual (pop) para pegar os 10% melhores
        ...
        while (newPop.size() < ga_params.popSize) {
            Individual ind;
           // if (randreal() < 0.5) {
              //  ind = make_simple_random_individual(irp);
           // } else {
                ind = make_simple_random_individual_2(irp);
          //  }
        ...
        // 4. Substituição da População
        pop.swap(newPop);

        // 5. Atualização do Melhor Global
        Individual& genBest = *std::min_element(pop.begin(), pop.end(), 
            [](const Individual& a, const Individual& b){ return a.fitness < b.fitness; });
        ...
        // 6. Lógica de Estagnação (REMOVIDA)
