...
EvaluationResult simulate_and_evaluate(const Individual& ind, const IRP& irp, const ACO_Params& aco_params) {
    ...
    for (int t = 0; t < irp.nPeriods; ++t) {
        ...
        // --- CÁLCULO NO DEPÓSITO ---
        depot_inv += irp.depots[0].production[t]; // 1. Produção chega

        // <-- MUDANÇA: Custo de armazenagem é calculado ANTES das entregas saírem
        if (depot_inv > 0) {
            result.depot_holding_cost += (double)depot_inv * irp.depots[0].invCost;
        }

        if (period_delivery_sum > depot_inv) { // 2. Checa se há estoque para entregar
            result.routing_cost = LARGE_COST;
            return result;
        }
        
        depot_inv -= period_delivery_sum; // 3. Entregas saem do depósito
        ...
        // --- CÁLCULO NOS CLIENTES ---
        for (int c = 0; c < irp.nCustomers; ++c) {
            customer_inv[c] += (long)del[c]; // 1. Entrega chega

            // <-- MUDANÇA: Custo de armazenagem é calculado ANTES da demanda ser consumida
            if (customer_inv[c] > 0) {
                result.customer_holding_cost += (double)customer_inv[c] * irp.customers[c].invCost;
            }

            customer_inv[c] -= (long)irp.customers[c].demand[t]; // 2. Demanda é consumida
    ...
    // A lógica de penalidade para estoque final (agora sem base no custo operacional) permanece
    long total_leftover_inventory = 0;
    for (int c = 0; c < irp.nCustomers; ++c) {
        if (customer_inv[c] > 0) {
            total_leftover_inventory += customer_inv[c];
    ...
    if (total_leftover_inventory > 0) {
        // A penalidade agora é um valor fixo por unidade para não ser subestimada
        result.final_inventory_penalty = 50.0 * total_leftover_inventory;
    }
